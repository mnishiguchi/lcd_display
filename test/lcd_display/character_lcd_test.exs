defmodule LcdDisplay.CharacterLcdTest do
  use ExUnit.Case
  ...
  test "start_link" do
    ...
    assert {:ok, pid} = LcdDisplay.CharacterLcd.start_link(config)

    # This does not guarantee what happens on the hardware but at least demonstrates how the API works.
    assert {:ok, _display} = LcdDisplay.CharacterLcd.execute(pid, {:print, "Hello"})
  end
end
